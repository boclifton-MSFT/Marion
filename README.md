...
The Service Bus client is registered once and shared by the publisher and readiness probe. The probe creates a fresh sender for each execution, opens an empty message batch, and never sends, receives, or peeks. Probe cleanup observes batch creation and sender disposal independently, and only one lifecycle may remain in flight; a stalled SDK operation therefore causes later readiness calls to fail fast instead of creating more stuck senders. For Azure deployments, grant the API identity the **Azure Service Bus Data Sender** role at the narrowest queue or namespace scope required by the publisher and probe. This repository's AppHost controls only the local emulator topology and does not create Azure role assignments; deployment provisioning must explicitly select that minimum role and must not rely on Aspire Azure resource defaults that may provision the broader **Azure Service Bus Data Owner** role.
