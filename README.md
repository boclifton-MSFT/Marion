...
The Service Bus client is registered once and shared by the publisher and readiness probe. The probe creates a fresh sender for each execution, opens an empty message batch, and never sends, receives, or peeks. Probe cleanup observes batch creation and sender disposal independently, and only one lifecycle may remain in flight; a stalled SDK operation therefore causes later readiness calls to fail fast instead of creating more stuck senders. Aspire's Azure Service Bus integration defaults references to the broader **Azure Service Bus Data Owner** role, so the AppHost explicitly replaces that default with **Azure Service Bus Data Sender** for the API's Service Bus reference. Aspire's publish model therefore emits only the sender role required by this publisher/probe path; the model assertion in `AppHostMessagingModelTests` guards that contract. For Azure deployments, keep the assignment at the narrowest namespace or queue scope supported by the deployment target and do not reintroduce the broader Data Owner role.
