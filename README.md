...
Azure mode requires a credential-free HTTPS Blob service root URI, Blob container name, Service Bus fully qualified namespace host, and SQL server and database names. Blob URIs containing user information, a query (including SAS parameters), a fragment, or a non-root path are rejected without echoing the configured value. Service Bus connection strings, URI schemes, paths, user information, queries, fragments, and secret-bearing forms are likewise rejected without echoing the input; sovereign, private, and custom hosts accepted by the Service Bus SDK remain valid. An optional managed identity client ID selects a user-assigned identity; leaving it unset uses the system-assigned identity. `TenantId` remains a compatibility setting but is not required or consumed by `ManagedIdentityCredential`. Azure mode registers one shared `ManagedIdentityCredential` for Blob, Service Bus, Key Vault, and SQL, while local mode registers no Azure credential. Validation runs at startup and reports only setting names and mode requirements, never configured values.
