...
Azure mode authenticates with `DefaultAzureCredential` (system-assigned managed identity by default, or the configured user-assigned client ID). It has no production connection-string, SAS-key, or shared-secret fallback; connection-string behavior is limited to the local emulator. The external namespace-wide sender assignment is the only production Service Bus authorization path.
