...
The AppHost models the `messaging` namespace, its `document-processing` queue, and its `loan-events` topic/subscription. `RunAsEmulator` keeps local runs on the Azure Service Bus emulator; publish mode uses the Azure Service Bus resource instead. Aspire's Service Bus reference defaults to the broader **Azure Service Bus Data Owner** role, so the AppHost explicitly replaces that default with **Azure Service Bus Data Sender** for the API reference. The publish model provisions the API's managed identity and only that sender assignment; the model assertion in `AppHostMessagingModelTests` guards the reference-to-role contract. The sender role is the production minimum for this publisher/readiness path, and deployments should scope it to the narrowest namespace or queue/topic resource they use.

Azure mode authenticates with `DefaultAzureCredential` (system-assigned managed identity by default, or the configured user-assigned client ID). It has no production connection-string, SAS-key, or shared-secret fallback; connection-string behavior is limited to the local emulator.
